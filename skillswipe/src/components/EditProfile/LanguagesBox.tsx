...
const LanguagesBox = () => {
   ...
   const [languageList, setLanguageList] = useState(profile.auth.languages as Language[]);
   ...
   return (
      ...
      >
         ...
         <div style={{ display: "flex", flexDirection: "column-reverse" }}>
            {languageList && languageList.map((language: Language, index: number) => (
               <div key={language.id}>
                  <Languages language={language} index={index+1} deleteLanguage={deleteLanguage} isNew={isNew(language)} />
               </div>
            ))}
         </div>
