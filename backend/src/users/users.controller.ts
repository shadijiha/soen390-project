...
import {
  ApplicationFileValidationPipe,
  ProfileImagesFileValidationPipe
...
export class UsersController {
  ...
  ): Promise<Users.GetUserByIdResponse> {
    try {
      const res: Users.GetUserByIdResponse = new Users.GetUserByIdResponse()
      res.user = await this.usersService.findOneById(id)
      res.connectionStatus = await this.connectionsService.getConnectionStatus(
        authedUser.id,
        id
      )
      res.connections = await this.connectionsService.getAcceptedConnections(
        res.user.id
      )
  ...
  async update (
    @AuthUser() authedUser: BearerPayload,
      ...
      @UploadedFiles(ProfileImagesFileValidationPipe)
  ...
  async updateStatus (
    @AuthUser() authedUser: BearerPayload,
      @Body() status: Users.UpdateStatusRequest
  ...
  async postDocuments (
    @AuthUser() authedUser: BearerPayload,
      ...
      @UploadedFiles(ApplicationFileValidationPipe)
  ...
  async removeDocuments (
    @AuthUser() authedUser: BearerPayload,
      @Body() data: Users.DeleteDocumentsRequest
  ): Promise<void> {
    const user = (await authedUser.getUser()) as User
  ...
  public async report (
    @AuthUser() authedUser: BearerPayload,
      @Body() data: Users.ReportRequest
  ): Promise<void> {
    const user = (await authedUser.getUser()) as User
    await this.usersService.report(user, data)
  }
}
