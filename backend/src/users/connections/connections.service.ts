...
export class ConnectionsService {
   ...
   */
  public async getPendingConnections(userId: number): Promise<{ user: User; since: Date }[]> {
   ...
   * @returns An array of objects with the following structure:
   */
  public async getAcceptedConnections(userId: number): Promise<{ user: User; since: Date }[]> {
    const connections = await this.connectionRepository
      .createQueryBuilder("connection")
      .leftJoinAndSelect("connection.user_1", "user_1")
      .leftJoinAndSelect("connection.user_2", "user_2")
   ...
   */
  public async acceptConnection(user1Id, user2Id): Promise<{ user: User; since: Date }[]> {
